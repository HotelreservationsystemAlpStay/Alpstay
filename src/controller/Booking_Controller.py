...
class Booking_Controller:
    ...
    def create_booking(self, user:User, room:Room, startDate:datetime, endDate:datetime):
        booking = Booking_Access().create_booking(0, check_in_date=startDate, check_out_date=endDate, is_cancelled=False, total_amount=1, guest_id=user.guest_id, room_id=room.room_id)
        if booking:
            return True
        return False
