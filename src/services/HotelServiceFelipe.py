...
def get_all_hotels():
    ...
    hotels = db_controller.fetchall(sql)
...
def search_hotels_by_city(city):
    ...
    WHERE Address.city = ?
    """
    hotels = db_controller.fetchall(sql, (city,))
...
search_hotels_by_city("Bern") 

# 1.2 Hotel nach Sterne anzeigen lassen

def search_hotels_by_city_and_stars(city, stars):
    sql = """
    ...
    AND Hotel.stars >= ?
    """
    hotels = db_controller.fetchall(sql, (city, stars,))
...
search_hotels_by_city_and_stars("Bern","3")

# 1.3 Hotels mit Zimmer für bestimmte Anzahl Gäste anzeigen lassen

def search_hotels_by_city_and_max_guests(city, max_guests):
    sql ="""
    ...
    """
    hotels = db_controller.fetchall(sql, (city, max_guests,))
...
search_hotels_by_city_and_max_guests("Zürich",5)
